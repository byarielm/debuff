...
dual_db_test!(
    create_definition_returns_201_with_locales,
    |backend| async move {
...
dual_db_test!(list_definitions_empty_by_default, |backend| async move {
    let app = common::app::TestApp::new(backend).await;
    ...
    assert_eq!(defs.len(), 0, "expected no definitions by default");
});
...
dual_db_test!(update_definition_severity, |backend| async move {
    ...
    let (status, body) = app
        .patch_authed(&format!("/api/definitions/{id}"), &update)
        .await;
...
dual_db_test!(
    invalid_identifier_uppercase_returns_400,
    |backend| async move {
        let app = common::app::TestApp::new(backend).await;
