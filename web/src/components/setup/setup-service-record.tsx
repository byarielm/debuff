...
export function SetupServiceRecord({ onComplete }: SetupServiceRecordProps) {
  ...
  const [reasonTypes, setReasonTypes] = useState<Set<string>>(
    new Set(REASON_TYPES.map((r) => r.id))
  )
  ...
  function handleAddCustomCollection() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(
        e instanceof Error ? e.message : "Failed to create service record"
      )
    } finally {
      setLoading(false)
    }
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <Button type="submit" disabled={loading}>
            {loading ? "Creating record..." : "Create Service Record"}
          </Button>
        </form>
      </CardContent>
    </Card>
