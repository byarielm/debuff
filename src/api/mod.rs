...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/moderators/{did}", delete(moderators::remove_moderator))
        .route(
            "/definitions",
        ...
        .route("/queue/{id}/assign", patch(queue::assign_moderator))
        .route("/queue/{id}/escalate", post(queue::escalate))
        .route("/queue/{id}/notes", post(queue::add_note))
        .route("/accounts/{did}", get(accounts::get_account))
        .route("/accounts/{did}/action", post(accounts::apply_action))
        .route(
            "/webhooks",
